...
function init() {
    ...
    let width = 3 * window.innerWidth / 5;
    ...
    selDegree.addEventListener('change', function () {
    ...
    pointxRange.addEventListener('input', function () {
...
function updatePointsTable() {
    ...
    for (let i = 0; i < nurbsControlPoints.length; ++i) {
        ...
        tableData += '<td><button onclick="deleteRow(' + i + ')">Delete</button></td>';
        tableData += '</tr>';
    }
...
function highlightSelectedRow() {
    for (let i = 0; i <= nurbsControlPoints.length; ++i) {
        document.getElementById('tablePointsBody').rows[i].className = "notselected";
    }
    document.getElementById('tablePointsBody').rows[selectedRow + 1].className = "selected";
}
...
function editRow(i) {
    ...
    showCurrentPoint(nurbsControlPoints[i].x, nurbsControlPoints[i].y, nurbsControlPoints[i].z);
    handleUValue();
}
...
function deleteRow(i) {
    if (nurbsControlPoints.length > minNoNurbsControlPoints) {
        ...
        alert("Cannot delete any further points; I need at least 6 Control Points to work.");
    }
}
...
function renderLineAndNurbsCurve() {
    ...
    for (let i = 0; i < nurbsControlPoints.length; ++i) {
        ...
        geometry.vertices.push(new THREE.Vector3(poi.x, poi.y, poi.z));
    }
